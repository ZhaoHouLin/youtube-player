...
export default {
  setup() {
    ...
    const handleOpen = ()=> {                       //判斷開啟輸入網址
      isOpen.value = !isOpen.value
    }

    const handleUrlVideoId = (arr)=> {              //處理youtube影片網址
      let filterVid = arr.filter((item)=> {         //將傳入的陣列篩選出除了'v='or'youtu.be/'的陣列
        return item.indexOf('v=') !== -1 || item.indexOf('youtu.be/') !== -1
      })
      store.dispatch('commitYtIdVideo',filterVid[0].split('v=')[1] || filterVid[0].split('youtu.be/')[1])                             //filterVid[0]是字串然後用'v='分段取後面的資料就是ID
      store.dispatch('commitIsRandom',false)
    }

    const urlGetId = ()=> {                         //從輸入的網址取得影片ID
      store.dispatch('commitIsRandom', false)
